/* aes.c - AES-128, encryption only */

#include "aes.h"

static const u8 sbox[256] = {
 };

static inline u8 xtime(u8 x) { return (u8)((x << 1) ^ ((x >> 7) * 0x1b)); }

void aes128_expand(AES128 *a, const u8 key[16])
{
    memcpy(a->rk[0], key, 16);
    u8 rcon = 1;
    for (int r = 1; r <= 10; r++) {
        const u8 *prev = a->rk[r - 1];
        u8 *cur = a->rk[r];
        cur[0] = prev[0] ^ sbox[prev[13]] ^ rcon;
        cur[1] = prev[1] ^ sbox[prev[14]];
        cur[2] = prev[2] ^ sbox[prev[15]];
        cur[3] = prev[3] ^ sbox[prev[12]];
        for (int i = 4; i < 16; i++) cur[i] = prev[i] ^ cur[i - 4];
        rcon = xtime(rcon);
    }
}

void aes128_encrypt(const AES128 *a, const u8 in[16], u8 out[16])
{
    u8 s[16];
    for (int i = 0; i < 16; i++) s[i] = in[i] ^ a->rk[0][i];
    for (int round = 1; round <= 10; round++) {
        u8 t[16];
        for (int i = 0; i < 16; i++) t[i] = sbox[s[i]];
        /* ShiftRows (column-major state: byte index = col*4 + row) */
        u8 r[16] = {
            t[0],  t[5],  t[10], t[15],
            t[4],  t[9],  t[14], t[3],
            t[8],  t[13], t[2],  t[7],
            t[12], t[1],  t[6],  t[11] };
        if (round != 10) {
            for (int c = 0; c < 4; c++) {
                u8 *p = r + c * 4;
                u8 a0 = p[0], a1 = p[1], a2 = p[2], a3 = p[3];
                u8 x = a0 ^ a1 ^ a2 ^ a3;
                u8 b0 = p[0], b1 = p[1], b2 = p[2], b3 = p[3];
                p[0] = a0 ^ x ^ xtime(b0 ^ b1);
                p[1] = a1 ^ x ^ xtime(b1 ^ b2);
                p[2] = a2 ^ x ^ xtime(b2 ^ b3);
                p[3] = a3 ^ x ^ xtime(b3 ^ b0);
            }
        }
        for (int i = 0; i < 16; i++) s[i] = r[i] ^ a->rk[round][i];
    }
    memcpy(out, s, 16);
}
